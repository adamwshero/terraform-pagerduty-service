...
resource "pagerduty_slack_connection" "service_reference" {
  for_each = var.create_slack_connection ? [true] : [] && var.source_type == "service_reference" ? 1 : 0
...
resource "pagerduty_slack_connection" "team_reference" {
  for_each = var.create_slack_connection ? [true] : [] && var.source_type == "team_reference" ? 1 : 0
